...
mission_spec = importlib.util.spec_from_file_location(
    mission_name, base / "mission_logger.py"
)
...
spec = importlib.util.spec_from_file_location("agents.razar.cli", base / "cli.py")
...
def test_bootstrap_invokes_retro_bootstrap(monkeypatch, capsys):
    ...
    monkeypatch.setattr(
        cli.retro_bootstrap, "bootstrap_from_docs", fake_bootstrap_from_docs
    )
