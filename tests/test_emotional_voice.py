...
def _expected_filters(history):
    ...
    return emotional_synaptic_engine.map_emotion_to_filters(
        history[-1]["emotion"], style=style
    )
...
def test_evolve_with_memory_updates_internal_styles(
    monkeypatch, tmp_path, mock_emotion_state
):
...
def test_speak_triggers_reflector(monkeypatch):
    ...
    dummy_reflector = types.SimpleNamespace(
        reflect=lambda p: events.setdefault("reflect", p)
    )
    monkeypatch.setitem(
