...
def test_speak_triggers_callback(monkeypatch):
    ...
    monkeypatch.setattr(
        expressive_output.language_engine, "synthesize_speech", fake_synth
    )
    monkeypatch.setattr(
        expressive_output.avatar_expression_engine,
        "stream_avatar_audio",
        lambda p, fps=15: iter(frames),
    )
    monkeypatch.setattr(
        expressive_output.audio_engine, "play_sound", lambda *a, **k: None
    )
