...
def test_modulate_voice_adds_preset_and_calls_synth(monkeypatch):
    ...
    assert (
        voice_evolution.get_voice_params("lunar")
        == voice_layer_albedo.TONE_PRESETS["lunar"]
    )


def test_modulate_voice_unknown_tone(monkeypatch):
    monkeypatch.setattr(
        voice_layer_albedo.speaking_engine, "synthesize_speech", lambda t, e: "x.wav"
    )
