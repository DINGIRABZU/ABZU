...
def test_dynamic_layer_selection(monkeypatch):
    ...
    monkeypatch.setattr(
        orchestrator.emotional_state, "get_current_layer", lambda: "dummy"
    )
    recorded = {}
    monkeypatch.setattr(
        orchestrator.emotional_state,
        "set_current_layer",
        lambda name: recorded.setdefault("layer", name),
    )
    ...
    monkeypatch.setattr(
        orchestrator.response_manager,
        "ResponseManager",
        lambda: type("R", (), {"generate_reply": lambda self, t, i: f"resp:{t}"})(),
    )
...
def test_invocation_task_sequence(monkeypatch):
    ...
    monkeypatch.setattr(
        orchestrator.qnl_engine, "parse_input", lambda t: {"tone": "joy"}
    )
    ...
    monkeypatch.setattr(
        MoGEOrchestrator, "route", lambda self, text, emotion_data, **k: {}
    )
    monkeypatch.setattr(
        orchestrator, "ritual_action_sequence", lambda sym, emo: ["open portal"]
    )
    monkeypatch.setattr(
        orchestrator.invocation_engine, "invoke", lambda t, o: [["weave sound"]]
    )
    monkeypatch.setattr(
...
def test_silence_invokes_introspection(monkeypatch):
    ...
    def fake_analyze(duration):
        return None, {
            "silence_meaning": "Extended silence – perhaps a pause for thought."
        }
    ...
    monkeypatch.setattr(
        orchestrator.invocation_engine, "invoke_ritual", fake_invoke_ritual
    )
    logged = {}
    monkeypatch.setattr(
        orchestrator,
        "log_ritual_result",
        lambda name, steps: logged.setdefault("steps", steps),
    )
