...
- [Documentation Index](index.md) – entry point for all guides
- [Project Overview](project_overview.md) – scope and goals of Spiral OS
- [Architecture Overview](architecture_overview.md) – high‑level component flow
- [Component Index](component_index.md) – per‑module descriptions and status
- [Recovery Playbook](recovery_playbook.md) – snapshot and restoration procedures
- [Documentation Index v2](INDEX.md) – searchable index generated by the doc indexer
- [Chakra Architecture](chakra_architecture.md) – layer responsibilities and heat
- [Memory Architecture](memory_architecture.md) – storage model and data flow
- [CROWN Overview](CROWN_OVERVIEW.md) – orchestration layer for LLM servants
...
- Failure Analyzer (`razar/issue_analyzer.py`) – inspects failure logs and
  labels them as dependency, logic, or external issues
- Quarantine Manager (`razar/quarantine_manager.py`) – moves failing modules to
  `quarantine/` and records the issue type and suggested fix in
