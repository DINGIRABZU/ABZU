...
- **Memory initialization diagnostics** instrument `scripts/bootstrap_memory.py` to emit Prometheus gauges that verify the 10 k-item audit workflow; dashboards and alert policies live in [monitoring/RAZAR.md](monitoring/RAZAR.md), and the readiness contract is detailed in [memory_layers_GUIDE.md](memory_layers_GUIDE.md#diagnostics-payload).
...
- **Stage B connector rehearsal** – The Stage B MCP adapters (`operator_api_stage_b.py`, `operator_upload_stage_b.py`, `crown_handshake_stage_b.py`) log rotation drills, heartbeat payloads, and rehearsal manifests referenced in the Stage gate evidence ledger so connector promotion reviews receive consistent artifacts. The rehearsal scheduler now also archives Stage B `/health` probes at `logs/stage_b/20250922T071749Z/health_checks.json`, capturing token usage, Stage context headers, and status codes for the operator and Crown connectors. The shared adapter now normalizes REST and trial gRPC handshakes, computes parity checksums, and persists a trace bundle per connector entry so the ledger mirrors the NeoABZU vector contract before the beta bridge review.【F:connectors/operator_mcp_adapter.py†L21-L170】【F:scripts/stage_b_smoke.py†L24-L230】【F:logs/stage_b_rotation_drills.jsonl†L1-L120】
- **Stage C continuity automation** – `operator_api` exposes Stage C controls (`POST /alpha/stage-c1-exit-checklist`, `/alpha/stage-c2-demo-storyline`, `/alpha/stage-c3-readiness-sync`, `/alpha/stage-c4-operator-mcp-drill`) so the dashboard can validate the exit checklist, capture scripted demo telemetry, merge Stage A/B readiness snapshots, and exercise the MCP handshake without leaving the console. Each call writes a timestamped run beneath `logs/stage_c/<run_id>/` with `summary.json`, stdout/stderr, and drill artifacts for evidence bundling. The Stage C4 drill now attaches the latest REST and gRPC handshake payloads, parity checksums, and a diff artifact annotated with credential expiry metadata to prove the rotation window stayed continuous between Stage B and Stage C rehearsals.【F:operator_api.py†L470-L713】【F:logs/stage_c/20251031T000000Z-test/rest_grpc_handshake_diff.json†L1-L12】
...
- [doctrine_index.md#docsroadmapmd](doctrine_index.md#docsroadmapmd) – docs/roadmap.md
- [doctrine_index.md#docsproject_statusmd](doctrine_index.md#docsproject_statusmd) – docs/PROJECT_STATUS.md
...
- **Stage A guardrails.** `scripts/run_alpha_gate.sh` collects build artifacts, health probes, and the mandated coverage invocation into `logs/alpha_gate/<timestamp>/`, then stamps the identity sync status from `python scripts/check_identity_sync.py`. Update [The Absolute Protocol](The_Absolute_Protocol.md#stage-gate-alignment) whenever guardrail scope changes and record bundle hashes in [doctrine_index.md](doctrine_index.md) to confirm provenance. Align each run with the ledger in [PROJECT_STATUS.md#stage-a-evidence-register](PROJECT_STATUS.md#stage-a-evidence-register); the successful 2025-09-20 rehearsal at 92.95 % coverage and the 2025-09-21 import-regression dry run both remain the canonical Stage A reference points and must stay cross-linked with [roadmap.md](roadmap.md#stage-a--alpha-gate-confidence) before Stage C planning locks. Operators can now invoke Stage A sweeps straight from the dashboard via `operator_api` (`POST /alpha/stage-a1-boot-telemetry`, `/alpha/stage-a2-crown-replays`, and `/alpha/stage-a3-gate-shakeout`), which archive stdout/stderr and `summary.json` under `logs/stage_a/<run_id>/` for the evidence bundle.
- **Stage B memory proof.** The load harness exports latency metrics and corpus checksums through `scripts/export_stage_b_artifacts.py`, writing structured outputs to `logs/stage_b/` and `monitoring/stage_b/` alongside dashboards referenced by the [Memory Bundle Architecture](#memory-bundle-architecture). Document dataset revisions, shard counts, and verification scripts in the Doctrine Index with each rehearsal snapshot, and mirror the expectations enumerated in [roadmap.md#stage-b--subsystem-hardening](roadmap.md#stage-b--subsystem-hardening) and [PROJECT_STATUS.md#stage-b-evidence](PROJECT_STATUS.md#stage-b-evidence).
- **Stage B sonic rehearsal.** Audio preflight runs `scripts/setup_audio_env.sh` and captures rehearsal manifests through `scripts/generate_stage_b_rehearsal_packet.py`, archiving `logs/stage_b_rehearsal_packet.json`, DAW availability results, and telemetry described in [monitoring/audio_rehearsal_telemetry.md](monitoring/audio_rehearsal_telemetry.md). Link preset or fallback changes back to the Doctrine Index so reviewers can replay the evidence chain. Current rehearsal packets continue to record missing FFmpeg, simpleaudio, CLAP, and RAVE dependencies, so the remediation work item called out in [PROJECT_STATUS.md#stage-c-planning-snapshot](PROJECT_STATUS.md#stage-c-planning-snapshot) must remain a blocking risk until the roadmap resolves it.
- **Stage C continuity drill.** Dashboard operators can now promote Stage C evidence with `operator_api` (`POST /alpha/stage-c1-exit-checklist`, `/alpha/stage-c2-demo-storyline`, `/alpha/stage-c3-readiness-sync`, `/alpha/stage-c4-operator-mcp-drill`). These endpoints validate the exit checklist, record scripted demo telemetry, merge Stage A/Stage B summaries, and capture MCP handshake/heartbeat payloads into `logs/stage_c/<run_id>/summary.json` alongside stdout/stderr for the ledger and doctrine cross-links.
- **Stage C continuity drill.** Dashboard operators can now promote Stage C evidence with `operator_api` (`POST /alpha/stage-c1-exit-checklist`, `/alpha/stage-c2-demo-storyline`, `/alpha/stage-c3-readiness-sync`, `/alpha/stage-c4-operator-mcp-drill`). These endpoints validate the exit checklist, record scripted demo telemetry, merge Stage A/Stage B summaries, and capture MCP handshake/heartbeat payloads into `logs/stage_c/<run_id>/summary.json` alongside stdout/stderr for the ledger and doctrine cross-links. The Mission Map groups these Stage A/B/C milestone controls under dedicated "Milestone Controls" sections so operators can replay `stage-a1`, `stage-a2`, `stage-b1`, `stage-b2`, `stage-b3`, `stage-c1`, and `stage-c2` directly from the console while streaming artifact summaries into the event log for evidence parity.【F:web_console/game_dashboard/dashboard.js†L180-L356】【F:web_console/game_dashboard/mission_map.js†L1-L144】【F:operator_api.py†L2184-L2421】
- **Stage D production bridge.** Stage D tasks replay the Stage C readiness bundle on hardware, align Neo-APSU crate fingerprints with the rehearsal ledger, and attach production transport diffs to the dashboards so weekly reviews inherit the bridge evidence.【F:docs/roadmap.md†L142-L168】【F:docs/PROJECT_STATUS.md†L143-L160】
- **Stage E transport readiness.** Stage E enforces REST↔gRPC parity as a beta gate, merges telemetry bundles for stakeholder dashboards, and records external sign-offs that hinge on the Stage D bridge ledger.【F:docs/roadmap.md†L170-L192】【F:docs/PROJECT_STATUS.md†L162-L181】
- **Evidence packaging policy.** Contributors must run `python scripts/package_evidence.py <bundle> <source> --upload-base s3://abzu-stage-b-rehearsals` for every rehearsal bundle, commit the resulting JSON manifest in `evidence_manifests/`, and publish the tarball outside Git. CI executes `scripts/check_evidence_manifests.py --require-upload-hint` so pull requests cannot merge without an upload URI and checksum trail. Reference the upload manifest list in [PROJECT_STATUS.md#stage-b-evidence](PROJECT_STATUS.md#stage-b-evidence) when closing Stage B tasks to keep the roadmap synchronized.
- **Stage B connector rotation.** Connector drills execute `scripts/stage_b_smoke.py`, emit `logs/stage_b_rotation_drills.jsonl`, and package heartbeat payloads per [Model Context Protocol Migration](#model-context-protocol-migration). Keep the rehearsal packet and rotation manifests synchronized with [mcp_connectors.md](mcp_connectors.md) and Stage B doctrine so promotion decisions inherit the same artifacts; the successive windows `20250926T180300Z-PT48H`, `20250926T180231Z-PT48H`, `20250925T095833Z-PT48H`, `20250925T094604Z-PT48H`, and `20251024T174210Z-PT48H` now sit alongside the earlier `20250922T101554Z-PT48H` smoke preflight, with artifacts cross-linked from [CONNECTOR_INDEX.md](connectors/CONNECTOR_INDEX.md) and the Stage B tracker in [PROJECT_STATUS.md#stage-b-evidence](PROJECT_STATUS.md#stage-b-evidence). Highlight skipped health automation in risk reviews per [PROJECT_STATUS.md#stage-c-planning-snapshot](PROJECT_STATUS.md#stage-c-planning-snapshot) so Stage C adoption inherits a mitigation plan. Stage B3 also appends the rotation headline and credential expiry to the ledger so Stage C readiness merges can display a single human-readable status without scraping stdout.【F:docs/mcp_connectors.md†L38-L58】【F:docs/PROJECT_STATUS.md†L82-L107】【F:operator_api.py†L470-L575】【F:operator_api.py†L640-L713】
- **Future gRPC adoption gate.** Promote the broader gRPC rollout only after Stage B memory, sonic, and connector bundles close, then mirror the NeoABZU vector service contract when exporting request/response traces, fallback manifests, and parity dashboards. Archive those artifacts beside the [Stage gate ledger](#stage-gate-evidence) entries referenced in [The Absolute Protocol](The_Absolute_Protocol.md#stage-gate-alignment) so reviewers can replay the evidence chain across documentation.
- The operator transport pilot now instruments REST and gRPC flows with shared latency, error, and fallback metrics plus span events, feeding the dashboard recommendations in `monitoring/operator_transport_pilot.md` and the parity tests that guard the pilot scope.【F:operator_api.py†L54-L374】【F:operator_api_grpc.py†L1-L148】【F:monitoring/operator_transport_pilot.md†L1-L39】【F:tests/test_operator_transport_contract.py†L1-L78】
...
| [crown_prompt_orchestrator.py](../crown_prompt_orchestrator.py) | [`MoGEOrchestrator` in `neoabzu_rag`](../NEOABZU/rag/src/lib.rs) | Environment-limited warning left the async pipeline stubbed during the Stage C bundle capture.【F:logs/stage_c/20250928T202834Z-stage_c3_readiness_sync/stage_b-b1-artifact1.stderr.log†L7-L15】 | Wire the Rust orchestrator routing so Stage D rehearsals exercise the full retrieval and ranking flow documented in the migration crosswalk.【F:NEOABZU/rag/src/lib.rs†L122-L177】【F:NEOABZU/docs/migration_crosswalk.md†L14-L21】 |
| [servant_model_manager.py](../servant_model_manager.py) | [`neoabzu_crown` servant bridge](../NEOABZU/crown/src/lib.rs) | Stage C ran with the “local registry only” sandbox stub, masking servant invocation telemetry.【F:logs/stage_c/20250928T202834Z-stage_c3_readiness_sync/stage_b-b1-artifact1.stderr.log†L11-L15】 | Replace the registry with the Rust bridge so MoGE-managed servants emit the metrics and validator hooks required by the Rust migration rules.【F:NEOABZU/crown/src/lib.rs†L102-L189】【F:docs/documentation_protocol.md†L5-L28】 |
| [state_transition_engine.py](../state_transition_engine.py) | [`neoabzu_crown::route_inevitability`](../NEOABZU/crown/src/lib.rs) | Deterministic rotation stub persisted through the Stage C readiness sync, leaving ritual gating untested.【F:logs/stage_c/20250928T202834Z-stage_c3_readiness_sync/stage_b-b1-artifact1.stderr.log†L5-L15】 | Implement the Rust inevitability journey output so Stage D rehearsals confirm the chakra transition contract.【F:NEOABZU/crown/src/lib.rs†L200-L210】【F:NEOABZU/docs/migration_crosswalk.md†L10-L18】 |
| [memory_store.py](../memory_store.py) | [`neoabzu_memory::MemoryBundle`](../NEOABZU/memory/src/lib.rs) | Readiness bundle recorded `cortex layer empty`, forcing the optional memory stub and zero ready layers during Stage C.【F:logs/stage_c/20250927T201020Z-stage_c3_readiness_sync/summary.json†L590-L627】 | Migrate writes and queries to the Rust bundle so all eight layers report ready status in Stage D parity runs.【F:NEOABZU/memory/src/lib.rs†L12-L120】【F:docs/documentation_protocol.md†L5-L28】 |
| [emotional_state.py](../emotional_state.py) | [`neoabzu_crown` expression options](../NEOABZU/crown/src/lib.rs) | Sandbox replay downgraded emotional state to an in-memory stub, skipping persisted aura updates.【F:logs/stage_c/20250928T202834Z-stage_c3_readiness_sync/stage_b-b1-artifact1.stderr.log†L9-L15】 | Anchor aura/voice selection to the Rust expression pipeline before Stage D to restore telemetry-backed state transitions.【F:NEOABZU/crown/src/lib.rs†L60-L197】【F:docs/documentation_protocol.md†L5-L28】 |
...
| --- | --- | --- | --- | --- | --- | --- |
| `crown_decider.py` | `neoabzu_crown::route_decision` | Replay the Stage B rotation ledger against the Stage C readiness bundle on gate-runner hardware before enabling the Rust decider, keeping connector windows aligned with Neo-APSU manifests.【F:logs/stage_b_rotation_drills.jsonl†L1-L40】【F:logs/stage_c/20251001T010101Z-readiness_packet/readiness_bundle/readiness_bundle.json†L1-L47】 | Wire REST↔gRPC contract tests and Grafana transport dashboards into roadmap/status updates so the Rust route emits continuous parity telemetry for beta planning.【F:tests/test_operator_transport_contract.py†L1-L210】【F:monitoring/operator_transport_pilot.md†L1-L66】【F:docs/roadmap.md†L229-L271】 | Carry the sandbox diffs into transport soak runs, logging checksum matches and heartbeat gaps in the Stage E readiness summary until latency feeds land.【F:logs/stage_e/20250930T121727Z-stage_e_transport_readiness/summary.json†L1-L48】 | Fold the Rust decider into Stage G gate-runner parity packets so rollback drills prove sandbox evidence is replayable on hardware before sign-off.【F:logs/stage_g/20251102T090000Z-stage_g_gate_runner_hardware/summary.json†L1-L13】 | Confirm the GA cutover inherits the Stage G ledger and telemetry hash when the Rust decider drives production routing.【F:logs/stage_h/20251115T090000Z-ga_hardware_cutover/summary.json†L1-L28】 |
| `crown_prompt_orchestrator.py` | `neoabzu_rag::MoGEOrchestrator` | Promote the orchestrator through the Stage D rollout checklist with Stage B connector rotations and Stage C parity traces to validate mission retrieval on hardware.【F:logs/stage_b_rotation_drills.jsonl†L1-L40】【F:logs/stage_c/20251031T000000Z-test/rest_grpc_handshake_diff.json†L1-L11】 | Attach orchestrator telemetry to the contract suite so REST/gRPC parity alerts bubble into roadmap risk notes and PROJECT_STATUS updates during beta prep.【F:tests/test_operator_transport_contract.py†L211-L320】【F:monitoring/operator_transport_pilot.md†L40-L84】 | Rehearse MoGE parity during soak weeks, capturing sandbox evidence consumption alongside the Stage E dashboard hash for later audits.【F:logs/stage_e/20250930T121727Z-stage_e_transport_readiness/summary.json†L20-L87】 | Include orchestrator checksum diffs in Stage G Neo-APSU parity artifacts so hardware rollbacks can fall back to sandbox traces without drift.【F:logs/stage_g/20251102T094500Z-stage_g_neo_apsu_parity/summary.json†L1-L13】 | GA hardware replay confirms orchestrator telemetry hash matches the Stage G source bundle before closing the ledger.【F:logs/stage_h/20251115T090000Z-ga_hardware_cutover/summary.json†L1-L28】 |
| `servant_model_manager.py` | `neoabzu_crown` servant bridge | Stage D rehearsals replay servant activation logs from the Stage C readiness packet so hardware runs inherit sandbox registry evidence when the Rust bridge lands.【F:logs/stage_c/20251001T010101Z-readiness_packet/readiness_bundle/readiness_bundle.json†L48-L120】 | Parity tests verify servant telemetry surfaces in REST/gRPC dashboards, and PROJECT_STATUS callouts note any sandbox deferrals that beta must absorb.【F:tests/test_operator_transport_contract.py†L1-L210】【F:monitoring/operator_transport_pilot.md†L1-L66】 | Soak cycles replay sandbox servant metrics until hardware emits the same counters, keeping deferrals visible in telemetry hashes.【F:logs/stage_e/20250930T121727Z-stage_e_transport_readiness/summary.json†L49-L87】 | Hardware parity packets document servant bridge rollbacks beside sandbox transcripts before declaring Stage G complete.【F:logs/stage_g/20251102T090000Z-stage_g_gate_runner_hardware/summary.json†L1-L13】 | GA cutover notes confirm servant parity metrics track the Stage G checksum lineage.【F:logs/stage_h/20251115T090000Z-ga_hardware_cutover/summary.json†L1-L28】 |
| `state_transition_engine.py` | `neoabzu_crown::route_inevitability` | Bridge rehearsals consume sandbox inevitability traces from the readiness packet so Stage D hardware runs demonstrate chakra transition continuity.【F:logs/stage_c/20251001T010101Z-readiness_packet/readiness_bundle/readiness_bundle.json†L121-L185】 | Contract suites and dashboards log inevitability transitions alongside transport parity metrics, keeping beta planning aware of any deferred heartbeat feeds.【F:tests/test_operator_transport_contract.py†L1-L210】【F:monitoring/operator_transport_pilot.md†L1-L84】 | Stage F soak runs monitor inevitability spans for drift, storing sandbox evidence links beside heartbeat remediation tasks.【F:logs/stage_e/20250930T121727Z-stage_e_transport_readiness/summary.json†L49-L87】 | Stage G parity bundles replay inevitability diffs with rollback instructions so auditors can trace sandbox origins of hardware approvals.【F:logs/stage_g/20251102T094500Z-stage_g_neo_apsu_parity/summary.json†L1-L13】 | GA summaries document inevitability telemetry as part of the production readiness hash lineage.【F:logs/stage_h/20251115T090000Z-ga_hardware_cutover/summary.json†L1-L28】 |
| `memory_store.py` | `neoabzu_memory::MemoryBundle` | Stage D parity imports Stage B rotations and Stage C readiness metrics before flipping hardware writes to the Rust bundle.【F:logs/stage_b_rotation_drills.jsonl†L1-L40】【F:logs/stage_c/20251001T010101Z-readiness_packet/readiness_bundle/readiness_bundle.json†L1-L185】 | Memory bundle checksums thread through parity dashboards so beta monitors ingestion latency and heartbeat gaps while sandbox counters persist.【F:tests/test_operator_transport_contract.py†L231-L320】【F:monitoring/operator_transport_pilot.md†L1-L84】 | Soak checkpoints validate the Rust bundle under sustained load, annotating dashboard hashes with sandbox evidence consumed for each replay.【F:logs/stage_e/20250930T121727Z-stage_e_transport_readiness/summary.json†L20-L87】 | Stage G summaries confirm memory checksums align with sandbox bundles before hardware promotion completes.【F:logs/stage_g/20251102T090000Z-stage_g_gate_runner_hardware/summary.json†L1-L13】 | GA cutover metrics prove the memory bundle parity hash persisted into production.【F:logs/stage_h/20251115T090000Z-ga_hardware_cutover/summary.json†L15-L28】 |
| `emotional_state.py` | `neoabzu_crown` expression pipeline | Hardware bridge replay leans on Stage B rotation audit trails and Stage C sandbox diffs to validate aura persistence before the Neo-APSU path activates.【F:logs/stage_b_rotation_drills.jsonl†L1-L40】【F:logs/stage_c/20251031T000000Z-test/summary.json†L1-L120】 | REST↔gRPC dashboards record expression state telemetry, ensuring beta planning tracks sandbox deferrals until hardware heartbeats ship.【F:tests/test_operator_transport_contract.py†L1-L210】【F:monitoring/operator_transport_pilot.md†L1-L66】 | Stage F monitors capture aura heartbeat gaps, archiving sandbox transcripts with each soak replay for Neo-APSU review.【F:logs/stage_e/20250930T121727Z-stage_e_transport_readiness/summary.json†L49-L87】 | Stage G parity runs annotate aura diffs in the Neo-APSU approvals packet, proving the sandbox evidence was consumed before hardware certification.【F:logs/stage_g/20251102T094500Z-stage_g_neo_apsu_parity/summary.json†L1-L13】 | GA ledger highlights expression telemetry convergence with the Stage G hardware source bundle.【F:logs/stage_h/20251115T090000Z-ga_hardware_cutover/summary.json†L1-L28】 |

#### Legacy→Neo-APSU alignment snapshot (2025-12)

- The APSU migration matrix now records sandbox expectations, hardware replay plans, and evidence maturity for every legacy entry point so reviewers can see which modules are running Neo-APSU codepaths today and which remain gated behind Codex-only shims.【F:docs/apsu_migration_matrix.md†L7-L23】
- Ported bridges (`crown_router`, `identity_loader.py`) execute the Rust path in sandbox and hardware contexts with Stage G approvals documenting parity, so no further rewrites are scheduled beyond continuing ledger verification.【F:component_index.json†L4-L21】【F:docs/apsu_migration_matrix.md†L9-L14】
- Pending rewrites (`crown_decider.py`, `crown_prompt_orchestrator.py`, `state_transition_engine.py`, `servant_model_manager.py`, `emotional_state.py`) remain sandbox-only implementations; Stage F hardware windows are queued to exercise the Rust surfaces against Stage B/C/E evidence bundles once access returns.【F:docs/apsu_migration_matrix.md†L15-L36】【F:docs/PROJECT_STATUS.md†L180-L187】【F:docs/stage_f_hardware_replay_plan.md†L9-L33】
- Wrapped shims (`memory_store.py`, `connectors/operator_mcp_adapter.py`) continue to run with sandbox caveats; hardware parity is tagged provisional until the Stage F replay rebuilds the Rust memory bundle and captures connector heartbeat metrics on the gate-runner hosts.【F:docs/apsu_migration_matrix.md†L37-L45】【F:docs/PROJECT_STATUS.md†L174-L205】【F:docs/stage_f_hardware_replay_plan.md†L33-L47】
...
| Neo-APSU Template | Stage B connector scaffold with capability payload schema | MCP – registers rehearsal contexts and rotation metadata | Logs Stage B handshakes and accepted contexts before emitting heartbeats. | 0.2.0 |
