...
- The refreshed APSU migration matrix now embeds parity fixtures and Stage runtime shim links for every legacy entry point so implementers can immediately locate the current behavior.【F:docs/apsu_migration_matrix.md†L7-L46】
- Ready bridges (`crown_router`, `identity_loader.py`) hold `ported` status in `component_index.json` and pair sandbox fixtures with Stage G approvals confirming Rust parity on hardware; only routine ledger verification remains.【F:component_index.json†L4-L21】【F:docs/apsu_migration_matrix.md†L9-L18】
- In-progress ports (`crown_decider.py`, `crown_prompt_orchestrator.py`, `state_transition_engine.py`, `servant_model_manager.py`, `emotional_state.py`) continue to run sandbox stubs registered in `scripts/_stage_runtime.py`; Stage F hardware slots will replay the Rust surfaces against Stage B/C/E evidence bundles before promotion.【F:docs/apsu_migration_matrix.md†L19-L36】【F:docs/PROJECT_STATUS.md†L180-L187】【F:docs/stage_f_hardware_replay_plan.md†L9-L33】
- Hardware-validation rows (`memory_store.py`, `connectors/operator_mcp_adapter.py`) stay `wrapped` while optional shims and transport contract tests operate in sandbox; Stage F/H replays rebuild Rust bundles and capture heartbeat telemetry on gate-runner hosts ahead of sign-off.【F:component_index.json†L77-L98】【F:docs/apsu_migration_matrix.md†L37-L46】【F:docs/stage_f_hardware_replay_plan.md†L33-L47】
