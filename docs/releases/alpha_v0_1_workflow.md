...
> [!WARNING]
> **Codex sandbox dependency limits.** Run the following pytest command only on the Stage A hardware profile. When the Codex sandbox blocks the hardware profile, annotate the skipped invocation with an `environment-limited` reason that mirrors the test output so reviewers can trace the deferred Stage A validation.
...
The consolidated run enforces the Stage A ≥90 % coverage bar against the module list explicitly targeted by the command. Logs and coverage artifacts generated by the gate must be archived under the timestamped directory created for the run (for example, `logs/alpha_gate/20250918T130119Z/`). Existing coverage output for comparison lives in `logs/alpha_gate/20250926T115603Z/tests.log`, which reviewers can consult before requesting another Stage A execution.
