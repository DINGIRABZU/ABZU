...
The consolidated run enforces the Stage A ≥90 % coverage bar against the module list explicitly targeted by the command. Logs and coverage artifacts generated by the gate must be archived under the timestamped directory created for the run (for example, `logs/alpha_gate/20250918T130119Z/`).
