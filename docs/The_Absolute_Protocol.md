...
- **Stage A guardrails validated** – Gate automation and coverage enforcement now ship as part of the Alpha gate audit bundle. Run `scripts/run_alpha_gate.sh` so packaging, health checks, and `pytest --cov` exports land in `logs/alpha_gate/<timestamp>/` for reviewers, mirroring the [Stage A roadmap commitments](roadmap.md#stage-a--alpha-gate-confidence). Coverage telemetry must remain ≥ 90 % through the feeds outlined in [system_blueprint.md#pytest-observability](system_blueprint.md#pytest-observability); record bundle hashes alongside the relevant entries in the [Doctrine Index](doctrine_index.md) so auditors can confirm evidence integrity.
- **Stage B memory fabric load proof** – Follow the [Stage B load objectives](roadmap.md#stage-b--subsystem-hardening) to demonstrate ≤ 120 ms P95 recalls on a 10 k-item dataset. Publish ingestion audit logs and latency charts while keeping the [Memory Bundle architecture](system_blueprint.md#memory-bundle-architecture) diagrams and associated Doctrine Index rows current.
- **Stage B sonic core rehearsal lock** – Enforce the pinned audio stack and DAW preflight rules described in [system_blueprint.md#audio-device](system_blueprint.md#audio-device) while completing two clean rehearsal exports with zero dropouts. Archive telemetry to satisfy the evidence trail promised in the [Stage B sonic core milestone](roadmap.md#stage-b--subsystem-hardening) and update the linked Doctrine Index entries when presets or guardrails change.
- **Stage B connector credential rotation** – Exercise the MCP rehearsal adapters so 48-hour rotation drills, smoke tests, and heartbeat payloads align with the [Stage B connector target](roadmap.md#stage-b--subsystem-hardening). Keep the [Model Context Protocol migration blueprint](system_blueprint.md#model-context-protocol-migration) synchronized with adapter logs and capture drill artifacts in the Doctrine Index to prove connector readiness.
