...
> - Mirror the scheduled reruns recorded in the Stage C1 exit checklist summary so roadmap, readiness bundle, and risk queue
>   owners reference the same gate-runner assignment and 2025-10-02 18:00 UTC hardware window.【F:logs/stage_c/20250930T210000Z-stage_c1_exit_checklist/summary.json†L1-L35】
- Reference the cross-team readiness review minutes whenever requesting hardware access outside the sandbox so the approval
...
> #### Stage G compliance checkpoints
> - Mirror the hardware parity runs logged in `logs/stage_g/20251102T090000Z-stage_g_gate_runner_hardware/` and `logs/stage_g/20251102T094500Z-stage_g_neo_apsu_parity/` when executing sandbox-to-hardware bridges so checksum diffs, rollback transcripts, and operator console exports are archived with the readiness bundle.【F:logs/stage_g/20251102T090000Z-stage_g_gate_runner_hardware/summary.json†L1-L13】【F:logs/stage_g/20251102T090000Z-stage_g_gate_runner_hardware/rollback_drill.md†L1-L24】【F:logs/stage_g/20251102T094500Z-stage_g_neo_apsu_parity/summary.json†L1-L13】
> - Confirm the approvals YAML files capture signatures from the operator lead, hardware owner (gate runner or Neo-APSU services), and QA reviewer before the hardware packet is considered complete.【F:logs/stage_g/20251102T090000Z-stage_g_gate_runner_hardware/approvals.yaml†L1-L12】【F:logs/stage_g/20251102T094500Z-stage_g_neo_apsu_parity/approvals.yaml†L1-L12】
> - Bundle the parity diffs with the Stage C readiness references (`logs/stage_c/20251001T010101Z-readiness_packet/`) and Stage B rotation ledger so auditors can trace the lineage from sandbox rehearsal to hardware execution without recomputing evidence.【F:logs/stage_c/20251001T010101Z-readiness_packet/review_minutes.md†L1-L44】【F:logs/stage_b_rotation_drills.jsonl†L24-L58】
> - Re-run documentation pre-commit hooks against updated doctrine (see `docs/documentation_protocol.md`) whenever Stage G artefacts change to keep roadmap, status, and doctrine index entries synchronized.

> #### Sandbox deferral ledger
> - Catalog every sandbox-only skip inside the readiness packet, Stage B rotation ledger, and roadmap entry so the hardware replay inherits the same command transcripts and telemetry hashes. The Stage C readiness bundle already marks packaging, coverage, and tooling gaps as `environment-limited`; mirror those notes when staging hardware reruns to prove deferred evidence is queued for gate-runner execution.【F:logs/stage_c/20251001T010101Z-readiness_packet/readiness_bundle/readiness_bundle.json†L1-L47】
> - When Neo-APSU migrations advance, append the specific Stage B rotation windows feeding the rollout (for example, `20250926T222814Z-PT48H`) and link the REST↔gRPC parity traces pulled from the Stage C trial so auditors can verify sandbox evidence was consumed before hardware sign-off.【F:logs/stage_b_rotation_drills.jsonl†L1-L40】【F:logs/stage_c/20251031T000000Z-test/rest_grpc_handshake_diff.json†L1-L11】
> - Record the sandbox dashboard hash and heartbeat gaps called out in the Stage E transport readiness summary alongside each deferral so reviewers can watch Grafana confirm when hardware replays close the loop.【F:logs/stage_e/20250930T121727Z-stage_e_transport_readiness/summary.json†L1-L87】【F:monitoring/operator_transport_pilot.md†L1-L84】

> #### Neo-APSU rollout checkpoints
> - Stage D bridge approvals must cite the Stage C readiness bundle, Stage B rotation ledger, and the Neo-APSU component they promote so doctrine crosswalks and hardware manifests show exactly which sandbox evidence was replayed before activating the Rust surface.【F:logs/stage_c/20251001T010101Z-readiness_packet/readiness_bundle/readiness_bundle.json†L1-L185】【F:logs/stage_b_rotation_drills.jsonl†L1-L40】【F:docs/system_blueprint.md†L93-L122】
> - Stage E transport governance requires embedding REST↔gRPC contract outcomes, Grafana parity snapshots, and MCP parity artifacts into roadmap and PROJECT_STATUS updates. Use those updates to document deferrals and note the hardware window that will consume the sandbox traces during Neo-APSU rollout reviews.【F:tests/test_operator_transport_contract.py†L1-L320】【F:monitoring/operator_transport_pilot.md†L1-L84】【F:docs/roadmap.md†L214-L286】
> - Stage G and Stage H summaries must preserve the telemetry hashes imported from sandbox evidence so GA sign-off proves the hardware replay matched the Neo-APSU bridge lineage.【F:logs/stage_g/20251102T094500Z-stage_g_neo_apsu_parity/summary.json†L1-L13】【F:logs/stage_h/20251115T090000Z-ga_hardware_cutover/summary.json†L1-L28】
...
- **Stage A guardrails validated** – Gate automation, coverage enforcement, and identity drift checks are now locked into the Alpha evidence bundle. Run `scripts/run_alpha_gate.sh` so packaging, health checks, `pytest --cov` exports, and the `python scripts/check_identity_sync.py` report land in `logs/alpha_gate/<timestamp>/`, matching the [Stage A roadmap commitments](roadmap.md#stage-a--alpha-gate-confidence). Maintain ≥ 90 % coverage through the feeds outlined in [system_blueprint.md#stage-gate-evidence](system_blueprint.md#stage-gate-evidence) and record bundle hashes in the [Doctrine Index](doctrine_index.md) to preserve the guardrail audit trail. The latest ledger (see [PROJECT_STATUS.md#stage-a-evidence-register](PROJECT_STATUS.md#stage-a-evidence-register)) confirms a successful 2025-09-20 rehearsal at 92.95 % coverage alongside the earlier 2025-09-21 dry run that logged an import regression while still capturing the audit bundle in `logs/alpha_gate/20250921T220258Z/`. Operators may now trigger the boot telemetry, replay capture, and gate shakeout directly from the console via `operator_api` (`POST /alpha/stage-a1-boot-telemetry`, `/alpha/stage-a2-crown-replays`, `/alpha/stage-a3-gate-shakeout`), which mirror the manual scripts and emit `logs/stage_a/<run_id>/summary.json` plus stdout/stderr for audit stitching. The Mission Map folds these Stage A endpoints together with the Stage B and Stage C milestones so the console buttons call the REST handlers asynchronously and echo the returned artifact paths in the event log for doctrine reconciliation.【F:web_console/game_dashboard/dashboard.js†L180-L356】【F:web_console/game_dashboard/mission_map.js†L1-L144】【F:operator_api.py†L2184-L2421】
...
- **Stage B connector rotation evidence** – Exercise the MCP rehearsal adapters so 48-hour rotation drills, smoke tests, and heartbeat payloads align with the [Stage B connector target](roadmap.md#stage-b--subsystem-hardening). Synchronize the [Model Context Protocol migration blueprint](system_blueprint.md#model-context-protocol-migration) with adapter logs and archive the drill manifests called out in [system_blueprint.md#stage-gate-evidence](system_blueprint.md#stage-gate-evidence) to prove connector readiness before promotion. Reference the refreshed rotation windows `20250926T180300Z-PT48H`, `20250926T180231Z-PT48H`, `20250925T095833Z-PT48H`, `20250925T094604Z-PT48H`, and `20251024T174210Z-PT48H` (alongside the earlier `20250922T101554Z-PT48H`) documented in [PROJECT_STATUS.md#stage-b-evidence](PROJECT_STATUS.md#stage-b-evidence) and [roadmap.md](roadmap.md#stage-b--subsystem-hardening) while Stage C owners finalize MCP adoption. Stage B3 now records a concise rotation headline and credential expiry in the ledger so Stage C readiness bundles and dashboards can surface the rotation state without parsing raw stdout.【F:docs/PROJECT_STATUS.md†L82-L107】【F:docs/roadmap.md†L70-L74】【F:operator_api.py†L470-L575】【F:operator_api.py†L640-L713】
- **Stage C continuity evidence** – Anchor readiness updates on the aggregated packet at `logs/stage_c/20251001T010101Z-readiness_packet/`, which stitches the refreshed Stage C3 readiness sync, Stage C1 checklist summary, and Stage C MCP drill outputs into a single review artifact. Track the hardware-only tasks (python packaging and coverage) scheduled for the 2025-10-02 18:00 UTC gate-runner-02 window, and surface the gRPC parity traces so integration owners inherit the connector rotation context during hardware replay.【F:logs/stage_c/20251001T010101Z-readiness_packet/readiness_bundle/readiness_bundle.json†L1-L185】【F:logs/stage_c/20250930T210000Z-stage_c1_exit_checklist/summary.json†L1-L35】【F:logs/stage_c/20251031T000000Z-test/summary.json†L1-L222】
- **Stage C sandbox addendum** – Reference [The Absolute Protocol Addendum — Stage C Sandbox Review](The_Absolute_Protocol_addendum.md) for the consolidated readiness packet snapshot, hardware rerun schedule, and MCP parity evidence captured after the cross-team review.【F:docs/The_Absolute_Protocol_addendum.md†L1-L14】
...
- **Future gRPC adoption gate** – Expand gRPC endpoints following the NeoABZU vector service handshake as the canonical reference. Trigger the rollout after Stage B evidence bundles in [roadmap.md](roadmap.md#stage-b--subsystem-hardening) close out, then publish mixed REST/gRPC rehearsal traces, parity validations, and operator sign-offs to the [stage gate ledger](system_blueprint.md#stage-gate-evidence) so Doctrine reviewers can trace the transition. The transport pilot instruments REST and gRPC with latency, error, and fallback metrics plus span annotations captured in the `operator_transport_pilot.md` dashboard runbook so readiness reviews inherit quantifiable parity evidence.【F:operator_api.py†L54-L374】【F:operator_api_grpc.py†L1-L148】【F:monitoring/operator_transport_pilot.md†L1-L39】
