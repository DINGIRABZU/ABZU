...
Stage D owners must reconcile the readiness ledger watch items (rotation ledger, readiness snapshot, and demo telemetry) before closing any roadmap risk, ensuring the documented `environment-limited` tags are cleared by the gate-runner replay and recorded in the doctrine updates that follow.【F:docs/readiness_ledger.md†L1-L32】 Use the [Stage D/E bridge review agenda](beta_bridge/agenda.md) to frame weekly check-ins and capture decisions directly against the readiness ledger, and collect incoming artifacts with the [Stage D/E evidence request template](beta_bridge/evidence_request_template.md) so submissions stay uniform across squads.
