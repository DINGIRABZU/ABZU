...
| **C4. Operator MCP drills** | @ops-team | Land `OperatorMCPAdapter` for `operator_api`/`operator_upload` and confirm Stage B parity with `crown_handshake`. | Run `scripts/stage_b_smoke.py` to log the 48-hour credential rotation drill for each connector and archive the results under `logs/stage_b_rotation_drills.jsonl`, ensuring the `20250922T101554Z-PT48H` window is called out for the next roadmap sync. | Promote the operator connectors to the MCP adoption checklist once smoke runs stabilize and Track B3 acceptance is closed out. |
